...
export const data = new SlashCommandBuilder()
  .setName("scry")
  .setDescription(
    "Scry a card's desired printing."
  )
  .addStringOption((opt) =>
    opt
      .setName("card_name")
...
function formatPriceLine(label: string, v?: string | null, prefix = "$"): string {
...
function buildPricesBlock(card: ScryfallCard): string {
  ...
  lines.push(
    [formatPriceLine("EUR", p.eur, "€"), formatPriceLine("Foil", p.eur_foil, "€")].join(
      "  |  "
    )
  );
 ...
 */
async function fetchByTcgplayerId(tcgplayerId: number): Promise<ScryfallCard | null> {
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    await interaction.editReply({
      content: "Timed out waiting for a selection. Run the command again.",
      components: [],
    });
    return;
  }
